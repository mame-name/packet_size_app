...
def main():
    with st.sidebar:
        ...
        with st.form("sim_form"):
            def input_row(label, placeholder=None, is_number=False, default_val=0.0):
                c1, c2 = st.columns([1, 2])
                with c1: st.markdown(f"<div style='padding-top:8px;'>{label}</div>", unsafe_allow_html=True)
                with c2:
                    if is_number: 
                        return st.number_input(label, value=default_val, step=1.0, format="%.2f", label_visibility="collapsed")
                    else: 
                        return st.text_input(label, placeholder=placeholder, label_visibility="collapsed")

            # 入力値の取得（デフォルト値を設定して0を防ぐ）
            i_w = input_row("　重量", is_number=True, default_val=200.0)
            i_sg = input_row("　比重", "0.000") # これは文字列で取得して後でfloat変換
            i_width = input_row("　巾", "折返し巾")
            i_length = input_row("　長さ", is_number=True, default_val=150.0)
        ...
        if submit:
            try:
                ...
                else: result_container.error("数値を正しく入力してください")
    ...
    if uploaded_file:
        try:
            # カラム読み込みの順序をExcelの構造に厳密に合わせる（AC列=28がシール）
            ...
            if not plot_df.empty:
                ...
                def add_trend(y_col, name, color):
                    # データの統計状況によりolsがエラーになるのを防ぐためtryを入れる
                    try:
